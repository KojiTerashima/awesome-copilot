...
これは `ax` コマンドが失敗したときだけ参照してください。これらの確認を先回りして実行してはいけません。

## まずバージョンを確認

`ax` がインストールされている場合（`command not found` ではない場合）、それ以上調査する前に必ず `ax --version` を実行してください。バージョンは `0.8.0` 以上である必要があります。多くのエラーはインストールが古いことが原因です。バージョンが古すぎる場合は、下の **バージョンが古すぎる** を参照してください。
