...
description: |
  AIエージェントのプラグイン、ツール、依存関係のサプライチェーン整合性を検証します。次のような場合にこのスキルを使用します:
  - エージェントプラグインやツールパッケージ向けに SHA-256 整合性マニフェストを生成する
  - インストール済みプラグインが公開済みマニフェストと一致することを検証する
  - エージェントツールディレクトリ内の改ざん・変更・未追跡ファイルを検出する
  - エージェントコンポーネントの依存関係ピン留めとバージョンポリシーを監査する
  - エージェントプラグイン昇格（dev → staging → production）の来歴チェーンを構築する
  - 「verify plugin integrity」「generate manifest」「check supply chain」「sign this plugin」のような依頼全般
---

# Agent Supply Chain Integrity

AIエージェントのプラグインとツール向けに整合性マニフェストを生成・検証します。改ざんを検出し、バージョンのピン留めを徹底し、サプライチェーンの来歴を確立します。

## 概要

エージェントプラグインや MCP サーバーには、npm パッケージやコンテナイメージと同じサプライチェーンリスクがあります。ただし、このエコシステムには npm provenance、Sigstore、SLSA に相当する仕組みがありません。このスキルはそのギャップを埋めます。
...
- プラグインを開発環境から本番環境へ昇格する前
- プラグイン PR のコードレビュー時
- レビュー後にファイルが変更されていないことを検証する CI ステップとして
- サードパーティ製エージェントツールや MCP サーバーを監査するとき
- 整合性要件のあるプラグインマーケットプレイスを構築するとき

---

## パターン 1: 整合性マニフェストの生成

すべてのプラグインファイルの SHA-256 ハッシュを含む、決定的な `INTEGRITY.json` を作成します。
...
エージェントの依存関係でバージョンがピン留めされているか確認します。
...
| Practice | Rationale |
|----------|-----------|
| **コードレビュー後にマニフェストを生成する** | レビュー済みコードと本番コードの一致を保証できる |
| **PR にマニフェストを含める** | レビュアーが何をハッシュ化したか検証できる |
| **デプロイ前に CI で検証する** | レビュー後の変更を検出できる |
| **改ざん証跡としてチェーンハッシュを使う** | 単一ハッシュでプラグイン全体の状態を表現できる |
| **ビルド成果物を除外する** | ソースファイルのみをハッシュ化する（.git、__pycache__、node_modules は除外） |
| **すべての依存関係バージョンをピン留めする** | ピン留めなしの依存関係 = インストールごとに異なるコード |
...
- [npm Provenance](https://docs.npmjs.com/generating-provenance-statements) — Sigstore ベースのパッケージ来歴
