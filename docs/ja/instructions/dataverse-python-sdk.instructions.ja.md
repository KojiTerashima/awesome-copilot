...
applyTo: '**'
---
# Dataverse SDK for Python — 公式クイックスタート

この instruction は、Dataverse SDK for Python (preview) に関する Microsoft Learn のガイダンスを要約し、そのまま使えるスニペットを提供します。

## 前提条件
- 読み取り/書き込み可能な Dataverse 環境
- Python 3.10+
- PyPI へのネットワーク アクセス
...
```
- HTTP 設定は省略可能です: `cfg.http_retries`, `cfg.http_backoff`, `cfg.http_timeout`。
